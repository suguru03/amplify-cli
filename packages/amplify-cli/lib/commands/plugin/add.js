...
async function resolvePluginPackagePath(context, inputPath) {
  ...
  let searchDirPaths = [
    constants_1.constants.ParentDirectory,
    constants_1.constants.LocalNodeModules,
    constants_1.constants.GlobalNodeModules,
    process.cwd(),
  ];
  ...
  const candicatePluginDirPaths = searchDirPaths
    .map(dirPath => path.normalize(path.join(scan_plugin_platform_1.normalizePluginDirectory(dirPath), inputPath)))
...
async function addNewPluginPackage(context, pluginDirPath) {
  try {
    const addUserPluginResult = await plugin_manager_1.addUserPluginPackage(context.pluginPlatform, pluginDirPath.trim());
    if (addUserPluginResult.isAdded) {
      context.print.success('Successfully added plugin package.');
      await plugin_manager_1.confirmAndScan(context.pluginPlatform);
    } else {
      context.print.error('Failed to add the plugin package.');
      context.print.info(`Error code: ${addUserPluginResult.error}`);
      if (
        addUserPluginResult.error === add_plugin_result_1.AddPluginError.FailedVerification &&
