  ...
  if (context.input.subCommands.length > 1) {
    ...
    const pluginNameQuestion = {
      ...
      validate: async input => {
        const pluginNameValidationResult = await verify_plugin_1.validPluginName(input);
...
complete execution of the amplify push command.`);
  context.print.red(amplify_event_1.AmplifyEvent.PrePull);
  context.print.green(`${indentationStr}${amplify_event_1.AmplifyEvent.PrePull} handler is invoked prior to the \
execution of the amplify pull command.`);
  context.print.red(amplify_event_1.AmplifyEvent.PostPull);
  context.print.green(`${indentationStr}${amplify_event_1.AmplifyEvent.PostPull} handler is invoked on the \
...
function updateAmplifyPluginJson(pluginDirPath, pluginName, pluginType, eventHandlers) {
  const filePath = path.join(pluginDirPath, constants_1.constants.MANIFEST_FILE_NAME);
