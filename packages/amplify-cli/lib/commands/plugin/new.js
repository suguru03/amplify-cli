...
async function plugIntoLocalAmplifyCli(context, pluginDirPath) {
  ...
  if (addPluginResult.isAdded) {
    ...
    if (
      addPluginResult.error === add_plugin_result_1.AddPluginError.FailedVerification &&
...
function printInfo(context, pluginDirPath, isPluggedInLocalAmplifyCLI) {
  ...
  const amplifyPluginJsonFilePath = path.normalize(path.join(pluginDirPath, constants_1.constants.MANIFEST_FILE_NAME));
