...
function moveBackendResourcesToCurrentCloudBackend(resources) {
  ...
  for (const resource of resources) {
    const sourceDir = path.normalize(
      path.join(amplify_cli_core_1.pathManager.getBackendDirPath(), resource.category, resource.resourceName),
    );
    const targetDir = path.normalize(
      path.join(amplify_cli_core_1.pathManager.getCurrentCloudBackendDirPath(), resource.category, resource.resourceName),
    );
    ...
    if (fs.pathExistsSync(sourceDir)) {
      ...
      if ((resource === null || resource === void 0 ? void 0 : resource.service) === 'Lambda') {
...
function updateamplifyMetaAfterResourceAdd(category, resourceName, metadataResource = {}, backendConfigResource, overwriteObjectIfExists) {
  ...
  update_backend_config_1.updateBackendConfigAfterResourceAdd(category, resourceName, backendConfigResource || metadataResource);
}
...
function updateamplifyMetaAfterResourceUpdate(category, resourceName, attribute, value) {
  ...
  if (['dependsOn', 'service'].includes(attribute)) {
    update_backend_config_1.updateBackendConfigAfterResourceUpdate(category, resourceName, attribute, value);
  }
  return updatedMeta;
}
...
async function updateamplifyMetaAfterPush(resources) {
  ...
  for (const resource of resources) {
    if (resource.serviceType !== 'imported') {
      ...
      if (fs.pathExistsSync(sourceDir)) {
        let hashDir = await resource_status_1.getHashForResourceDir(sourceDir);
        if (resource.category === 'hosting' && resource.service === 'ElasticContainer') {
          ...
          if (projectRootPath) {
            ...
            hashDir = await resource_status_1.getHashForResourceDir(sourceAbsolutePath, [
              'Dockerfile',
              'docker-compose.yaml',
              'docker-compose.yml',
            ]);
...
function updateamplifyMetaAfterBuild({ category, resourceName }, buildType = amplify_function_plugin_interface_1.BuildType.PROD) {
  const amplifyMeta = amplify_cli_core_1.stateManager.getMeta();
  lodash_1.default.set(amplifyMeta, [category, resourceName, amplify_category_function_1.buildTypeKeyMap[buildType]], new Date());
  amplify_cli_core_1.stateManager.setMeta(undefined, amplifyMeta);
}
exports.updateamplifyMetaAfterBuild = updateamplifyMetaAfterBuild;
function updateAmplifyMetaAfterPackage({ category, resourceName }, zipFilename) {
  const amplifyMeta = amplify_cli_core_1.stateManager.getMeta();
  lodash_1.default.set(amplifyMeta, [category, resourceName, 'lastPackageTimeStamp'], new Date());
  lodash_1.default.set(amplifyMeta, [category, resourceName, 'distZipFilename'], zipFilename);
  amplify_cli_core_1.stateManager.setMeta(undefined, amplifyMeta);
}
