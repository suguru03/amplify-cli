...
const postInstallInitialization = async () => {
  context_extensions_1.print.info(`Initializing new Amplify CLI version...`);
      ...
      await Promise.all(
        pluginArtifactPaths.map(assetPath =>
          fs.copy(
            ...
            path.join(amplify_cli_core_1.pathManager.getAmplifyPackageLibDirPath(packageName), assetPath),
          ),
        ),
      );
    }),
  ...
  context_extensions_1.print.success('Done initializing new version.');
};
