...
function normalizeInput(input) {
  if (input.options) {
    ...
    if (input.options[constants_1.constants.YES] === undefined) {
      input.options[constants_1.constants.YES] = false;
    }
  }
...
function verifyInput(pluginPlatform, input) {
  ...
  if (pluginCandidates.length > 0) {
    for (let i = 0; i < pluginCandidates.length; i++) {
      ...
      if (
        (commands && commands.includes(constants_1.constants.HELP)) ||
        (commandAliases && Object.keys(commandAliases).includes(constants_1.constants.HELP))
      ) {
