...
function removeGetUserEndpoints(resourceName) {
  ...
  if (fs.existsSync(pinpointTemplateFilePath)) {
    let pinpointTemplateFile = amplify_cli_core_1.JSONUtilities.readJson(pinpointTemplateFilePath);
    const unAuthAction = lodash_1.default.get(
      pinpointTemplateFile,
      'Resources.CognitoUnauthPolicy.Properties.PolicyDocument.Statement[0].Action',
    );
    const authAction = lodash_1.default.get(
      pinpointTemplateFile,
      'Resources.CognitoAuthPolicy.Properties.PolicyDocument.Statement[0].Action',
    );
    ...
    lodash_1.default.set(pinpointTemplateFile, 'Resources.CognitoUnauthPolicy.Properties.PolicyDocument.Statement[0].Action', unAuthAction);
    lodash_1.default.set(pinpointTemplateFile, 'Resources.CognitoAuthPolicy.Properties.PolicyDocument.Statement[0].Action', authAction);
