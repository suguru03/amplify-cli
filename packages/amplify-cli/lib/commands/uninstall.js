...
const run = async context => {
  ...
  if (
    !((_b = (_a = context === null || context === void 0 ? void 0 : context.input) === null || _a === void 0 ? void 0 : _a.options) ===
      null || _b === void 0
      ? void 0
      : _b.yes) &&
  ...
  if (process.platform.startsWith('win')) {
    ...
    try {
      ...
      throw new Error(
        `Unable to move binary out of .amplify directory. You can manually remove [${amplify_cli_core_1.pathManager.getHomeDotAmplifyDirPath()}]`,
      );
    }
    try {
      hidefile_1.hideSync(win_constants_1.pendingDeletePath);
    } catch (err) {}
    try {
      await win_utils_1.setRegPendingDelete();
    } catch (err) {
      context.print.warning(err);
      context.print.warning(
        `Unable to set registry value marking Amplify binary for deletion. You can manually delete ${win_constants_1.pendingDeletePath}.`,
      );
    }
  }
...
const removeHomeDotAmplifyDir = async () => {
  try {
    ...
    throw new Error(
      `Failed to remove [${amplify_cli_core_1.pathManager.getHomeDotAmplifyDirPath()}]\nYou'll need to manually remove this directory.`,
    );
  }
};
