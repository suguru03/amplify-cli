...
const addTrigger = async triggerOptions => {
  ...
  if (values && values.length > 0) {
    for (let v = 0; v < values.length; v += 1) {
      await exports.copyFunctions(key, values[v], category, context, targetPath);
    }
  }
  const result = {};
...
const updateTrigger = async triggerOptions => {
  ...
  try {
    ...
    if (values && values.length > 0) {
      ...
      await exports.cleanFunctions(key, values, category, context, targetPath);
    }
...
const triggerFlow = async (context, resource, category, previousTriggers = {}) => {
  ...
  const triggerOptions = exports.choicesFromMetadata(triggerPath, resource, true);
  ...
  if (askTriggers.triggers) {
    for (let i = 0; i < askTriggers.triggers.length; i++) {
      ...
      const templateOptions = exports.choicesFromMetadata(optionsPath, askTriggers.triggers[i]);
...
const learnMoreLoop = async (key, map, metaData, question) => {
  ...
  while (Array.isArray(selections[key]) && selections[key].includes('learn')) {
    let prefix;
    if (metaData.URL) {
      prefix = `\nAdditional information about the ${key} available for ${map} can be found here: ${chalk_1.default.blue.underline(
        metaData.URL,
      )}\n`;
...
const choicesFromMetadata = (triggerPath, selection, isDir) => {
  ...
  const metaData = exports.getTriggerMetadata(triggerPath, selection);
