...
const binUrl = (version, binName) => `https://github.com/${repoOwner}/${repoName}/releases/download/v${version}/${binName}.tgz`;
...
const run = async context => {
  ...
  if (semver_1.gt(latestVersion, thisVersion)) {
...
const upgradeCli = async (print, version) => {
  ...
  await new Promise((resolve, reject) =>
    response.body
      ...
      .pipe(gunzip_maybe_1.default())
      .pipe(tar_fs_1.default.extract(binDir))
      .on('finish', resolve)
      .on('error', reject),
  );
...
const getLatestVersion = async () => {
  const response = await node_fetch_1.default(latestVersionUrl);
