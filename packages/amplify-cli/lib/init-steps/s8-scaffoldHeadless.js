...
async function scaffoldProjectHeadless(context) {
  ...
  const projectConfigFile = amplify_cli_core_1.JSONUtilities.readJson(
    path.join(skeletonLocalDir, amplify_cli_core_1.PathConstants.DotConfigDirName, `project-config__${frontend}.json`),
  );
  ...
  await fs.copy(
    path.join(skeletonLocalDir, amplify_cli_core_1.PathConstants.BackendDirName),
    amplify_cli_core_1.pathManager.getBackendDirPath(projectPath),
  );
  git_manager_1.insertAmplifyIgnore(amplify_cli_core_1.pathManager.getGitIgnoreFilePath(projectPath));
