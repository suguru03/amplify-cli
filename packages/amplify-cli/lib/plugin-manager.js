...
Object.defineProperty(exports, 'verifyPlugin', {
  enumerable: true,
  get: function () {
    return verify_plugin_1.verifyPlugin;
  },
});
...
async function getPluginPlatform() {
  let pluginPlatform = access_plugins_file_1.readPluginsJsonFile();
  if (pluginPlatform) {
    if (isCoreMatching(pluginPlatform)) {
      ...
      await post_install_initialization_1.postInstallInitialization();
      pluginPlatform = await scan();
    }
  } else {
    await post_install_initialization_1.postInstallInitialization();
...
async function scan(pluginPlatform) {
  ...
  try {
    const result = await scan_plugin_platform_1.scanPluginPlatform(pluginPlatform);
    context_extensions_1.print.info('Plugin scan successful');
    return result;
  } catch (e) {
    context_extensions_1.print.error('Plugin scan failed.');
    context_extensions_1.print.info(e);
...
const addUserPluginPackage = async (pluginPlatform, pluginDirPath) => {
  return exports.addPluginPackage(pluginPlatform, pluginDirPath);
};
exports.addUserPluginPackage = addUserPluginPackage;
const addExcludedPluginPackage = async (pluginPlatform, pluginInfo) => {
  return exports.addPluginPackage(pluginPlatform, pluginInfo.packageLocation);
};
...
const addPluginPackage = async (pluginPlatform, pluginDirPath) => {
  ...
  if (pluginVerificationResult.verified) {
    ...
    if (
      !scan_plugin_platform_1.isUnderScanCoverageSync(pluginPlatform, pluginDirPath) &&
