...
async function analyzeProject(context) {
  ...
  context.print.info('');
  await s0_analyzeProject_1.displayConfigurationDefaults(context, projectName, envName, defaultEditor);
...
function displayContainersInfo(context) {
  ...
  const containerDeploymentStatus = execution_manager_1.isContainersEnabled(context) ? 'Yes' : 'No';
