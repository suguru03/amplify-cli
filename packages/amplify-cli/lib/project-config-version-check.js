...
async function checkProjectConfigVersion(context) {
  ...
  if (projectPath) {
    ...
    const currentProjectVersion = semver_1.coerce(projectConfig.version);
    const minProjectVersion = semver_1.coerce(constants.MIN_NODE12_PROJECT_CONFIG_VERSION);
    ...
    if (semver_1.lt(currentProjectVersion, minProjectVersion)) {
...
async function checkLambdaCustomResourceNodeVersion(context, projectPath) {
  ...
  if (filesToUpdate.length > 0) {
    const confirmed =
      ((_a = context.input.options) === null || _a === void 0 ? void 0 : _a.yes) || (await promptForConfirmation(context, filesToUpdate));
    if (confirmed) {
...
async function checkFileContent(filePath) {
  const { cfnTemplate } = await amplify_cli_core_1.readCFNTemplate(filePath);
  const resources = lodash_1.default.get(cfnTemplate, 'Resources', {});
  const lambdaFunctions = lodash_1.default.filter(
    resources,
    r =>
      r.Type === 'AWS::Lambda::Function' &&
      previousLambdaRuntimeVersions.includes(lodash_1.default.get(r, ['Properties', 'Runtime'], undefined)),
  );
  return lambdaFunctions.length > 0;
}
async function updateFileContent(filePath) {
  const { templateFormat, cfnTemplate } = await amplify_cli_core_1.readCFNTemplate(filePath);
  const resources = lodash_1.default.get(cfnTemplate, 'Resources', {});
  const lambdaFunctions = lodash_1.default.filter(
    resources,
    r =>
  ...
  return amplify_cli_core_1.writeCFNTemplate(cfnTemplate, filePath, { templateFormat });
}
