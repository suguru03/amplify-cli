...
const setRegPendingDelete = async () => {
  ...
  let regQueryOutput = '';
  try {
    ({ stdout: regQueryOutput } = await execa_1.command(`reg query "${regPath}" /v ${regKey}`, { shell: 'cmd.exe' }));
  } catch (err) {}
  ...
  await execa_1.command(`reg import "${win_constants_1.tmpRegPath}"`, { shell: 'cmd.exe' });
