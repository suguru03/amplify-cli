...
async function migrateProject(context) {
  ...
  const minProjectVersion = semver_1.coerce(constants_1.amplifyCLIConstants.MIN_MIGRATION_PROJECT_CONFIG_VERSION);
  if (semver_1.lt(currentProjectVersion, minProjectVersion)) {
    if (await context.prompt.confirm(confirmMigrateMessage)) {
      const infoMessage =
        `${chalk_1.default.bold('The CLI is going to take the following actions during the migration step:')}\n` +
...
function backup(amplifyDirPath, projectPath) {
  const backupAmplifyDirName = `${constants_1.amplifyCLIConstants.AmplifyCLIDirName}-${make_id_1.makeId(5)}`;
...
function generateMigrationInfo(projectConfig, projectPath) {
  ...
  migrationInfo.amplifyMeta = amplify_cli_core_1.stateManager.getMeta(projectPath);
  migrationInfo.currentAmplifyMeta = amplify_cli_core_1.stateManager.getCurrentMeta(projectPath);
