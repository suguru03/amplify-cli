...
exports.run = run;
async function removeNamedPlugins(pluginPlatform, pluginInfos) {
  if (pluginInfos.length === 1) {
    ...
    if (selections.length > 0) {
      const sequential = require('promise-sequential');
      const removeTasks = selections.map(pluginInfo => async () => {
        await plugin_manager_1.removePluginPackage(pluginPlatform, pluginInfo);
      });
      await sequential(removeTasks);
    }
  }
